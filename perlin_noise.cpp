// Code Obtained from ChatGPT with the following prompt:
// How do I generate perlin noise using c++ and opengl
...
float perlinNoise(float x, float y, const std::vector<int>& permutation) {
    // Find unit square
    int X = static_cast<int>(floor(x)) & 255;
    int Y = static_cast<int>(floor(y)) & 255;

    // Relative x, y within the square
    x -= floor(x);
    y -= floor(y);
...
std::vector<int> generatePermutation() {
    return { 151, 160, 137,  91,  90,  15, 131,  13, 201,  95,  96,  53, 194, 233,   7, 225,
